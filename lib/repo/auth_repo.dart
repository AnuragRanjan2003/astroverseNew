...
class AuthRepo{
  ...
  Future<Resource<UserCredential>> createUser(models.User user , String password) async => await _auth.signupWithEmail(user, password);
   ...
   Future<Resource<void>> saveUserData(models.User user) async => await _db.saveUserData(user);

   Future<Resource<String>> storeProfileImage(File file , String id) async => await _store.storeProfileImage(file, id);
